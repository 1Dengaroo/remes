      ...
      <div className="space-y-3">
        ...
        {STEPS.map((step) => (
          <div key={step.label} className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_1fr]">
            {/* Without */}
            <div className="border-border bg-card rounded-xl border px-5 py-4 shadow-(--landing-shadow-card)">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <LogoRow logos={step.logos} />
                  <span className="text-foreground text-sm font-medium">{step.label}</span>
                </div>
                <span className="text-muted-foreground text-2xs shrink-0 rounded-full bg-(--landing-skel-base) px-2 py-0.5 font-medium">
                  ~{step.timeBefore}
                </span>
              </div>
              <p className="text-muted-foreground mt-2 text-xs leading-relaxed">{step.without}</p>
            </div>
